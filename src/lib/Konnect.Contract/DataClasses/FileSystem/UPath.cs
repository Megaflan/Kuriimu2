...
// - Removed usage of the shared string builder
...
namespace Konnect.Contract.DataClasses.FileSystem
{
    ...
    public struct UPath : IEquatable<UPath>, IComparable<UPath>
    {
        ...
        public UPath(string path, bool safe)
        {
            ...
            {
                FullName = ValidateAndNormalize(path, out var errorMessage);
        ...
        /// <summary>
        /// Performs an explicit conversion from <see cref="UPath"/> to <see cref="string"/>.
        /// </summary>
        ...
        /// path2
        /// </exception>
        /// <exception cref="ArgumentException">If an error occurs while trying to combine paths.</exception>
        ...
        /// <summary>
        /// Implements the / operator equivalent of <see cref="Combine(UPath,UPath)"/>
        /// </summary>
        /// <param name="path1">The first path to combine.</param>
        /// <param name="path2">The second path to combine.</param>
        /// <returns>The combined paths. If one of the specified paths is a zero-length string, this method returns the other path. If path2 contains an absolute path, this method returns path2.</returns>
        ...
        private static string ValidateAndNormalize(string path, out string errorMessage)
        {
            ...
            try
            {
                ...
                for (; i < path.Length; i++)
                {
                    ...
                    // We don't disallow characters, as we let the IFileSystem implementations decide for them
